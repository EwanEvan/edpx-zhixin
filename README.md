...
##### 使用 x-template 编写前端模板
和utpl使用方法类似，不同点如下，1）支持smarty修改器的语法 2）可以在模板中配置编译函数名称

_page.tpl类似 `{%*include file="./ajax-item.x-template"*%}` 模板类似
```javascriptt
<!--x-template:{"innerLang":1, "variable": "html", "prefix": "this.renderItem"}-->
<%if (list.length){%>
    <%for(var i = 0; i < list.length; i++){%>
        <a href="<%list[i].url|escape:'html'%>"><%=item.text%></a>
    <%}%>
<%}else{%>
    <dd class="cl-more">
        <%item.customCallback("查看全部特卖")%>
    </dd>
<%}%>
```
编译结果如下
```javascript
cache.render=function(data){var _={};_.escape=function(e,t){return"html"==t?this.encode(e):"url"==t?encodeURIComponent(e):"js"==t?this.escapeJs(e):e},_.escapeJs=function(){alert("TODO")},_.trim=function(e){e=_.init(e).replace(/^\s+/,"");for(var t=e.length-1;t>=0;t--)if(/\S/.test(e.charAt(t))){e=e.substring(0,t+1);break}return e},_.init=function(e){return("undefined"==typeof e||null===e)&&(e=""),""+e},_.cat=function(){var e=[].slice.call(arguments);return e.join("")},_.encodeURIComponent=function(e){return e=_.init(e),encodeURIComponent?encodeURIComponent(e):escape(e)},_.encode=function(e){return e=_.init(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/\x3E/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")},_.raw=function(e){return e},_.limitlen=function(e,t,s){for(var i=0,c="",a=/[^\x00-\xff]/g,n="",l=e.replace(a,"**").length,r=0;l>r&&(n=e.charAt(r).toString(),null!=n.match(a)?i+=2:i++,!(i>t));r++)c+=n;return s&&l>t&&(c+=s),c};var s="";with(data)if(s+='<!--x-template:{"innerLang":1, "variable": "s", "prefix": "cache.render"}-->',list.length)for(var i=0;i<list.length;i++){var item=list[i];s+='<li class="cl-item moz-clear" data-click="{rsv_click_mid:\'',s+=_.encode(item.mid),s+="',rsv_click_index: ",s+=_.encode(i),s+='}"><a class="ul-logo EC_ZHIXIN" target="_blank" href="',s+=_.encode(item.url),s+='" data-click="{rsv_click_type:\'item_image\'}"><img src="',s+=_.encode(item.brandlogo),s+='" width="96" height="48" alt="',s+=_.encode(item.title),s+='"/></a><div class="ul-misc"><div class="ul-misc-title moz-overflow" data-click="{rsv_click_type:\'item_title\'}"><a class="EC_ZHIXIN" target="_blank" href="',s+=_.encode(item.url),s+='">',s+=_.encode(item.title),s+='</a></div><a class="ul-misc-slo moz-overflow EC_ZHIXIN" target="_blank" href="',s+=_.encode(item.url),s+='" data-click="{rsv_click_type:\'item_promotion\'}">',item.left_day>0&&(s+='<span class="ul-misc-slo-time">',s+=_.encode(item.lasttime),s+="</span>"),s+='<span class="ul-misc-slo-promotion moz-dummy">',s+=_.escape(item.promotion,"none"),s+="</span></a>",item.source&&(s+='<div class="ul-misc-platform">商家：<a class="EC_ZHIXIN moz-dummy" target="_blank" href="',s+=_.encode(platMore),s+="&platform=",s+=_.encodeURIComponent(item.source),s+='" data-click="{rsv_click_type:\'item_source\'}">',s+=_.escape(item.source,"none"),s+="</a>",item.yhq&&(s+='<a target="_blank" href="',s+=_.encode(item.yhq),s+='" data-click="{rsv_click_type:\'item_quan\'}" class="c-text c-text-danger EC_ZHIXIN" title="优惠券">券</a>'),s+="</div>"),s+="</div>",item.btntext||(item.btntext=4==item.material_type?"进入店铺":"立即抢购"),s+='<div class="ul-purchase"><a class="ul-rebate moz-dummy EC_ZHIXIN" target="_blank" href="',s+=_.encode(item.url),s+='" data-click="{rsv_click_type:\'item_rebate\'}">',s+=_.raw(item.rebate),s+='</a><a class="c-btn c-btn-primary EC_ZHIXIN" target="_blank" href="',s+=_.encode(item.url),s+='" title="',s+=_.escape(item.btntext,"html"),s+='" data-click="{rsv_click_type:\'item_button\'}">',s+=_.encode(item.btntext),s+="</a></div></li>"}else s+='<li class="cl-more"><i href="#" class="c-icon c-icon-warning-circle-gray"></i>抱歉没有找到符合条件的信息，请尝试其他分类<br/ ><a href="',s+=_.encode(seeMore),s+='" target="_blank" data-click="{rsv_click_type:\\\'more\\\'}">查看全部特卖>></a></li>';return s};

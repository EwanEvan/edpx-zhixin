...
var xTemplate = require('./x-template');
 ...
 * @param  {Object} opts 编译参数
 */
function buildXtemplate(filePath, fileName, opts) {
    var hash = util.getPathHash(filePath);
    var defaul = '';

    try {
        var tpl = fs.readFileSync(filePath, 'utf-8');
        var conf = xTemplate.parseConfig(tpl) || {};
        conf = xTemplate.extend(conf, {
            innerLang: 1,
            prefix: 'var render2'
        }, false);

        defaul = xTemplate.generate(tpl, conf);
        if(opts.r) {
            var UglifyJS = require('uglify-js');
            defaul = UglifyJS.minify(defaul, {
                fromString: true
            }).code;
        }
            ...
            if (fs.existsSync(filePath)) {
                switch (path.extname(filePath).toLowerCase()) {
                    ...
                    case '.utpl':
                        ...
                        break;
                    case '.x-template':
                        buildXtemplate(filePath, filename, opts);
                        break;
                    default:
                        break;
                }
